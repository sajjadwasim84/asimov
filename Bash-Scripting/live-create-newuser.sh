It's Tuesday December 17, 2019

FILE SYSTEMS USAGE SNAPSHOT
Home is at    39% usage of 32T
Scratch is at 63% usage of 116T
Data is at    34% usage of 55T

TODAY'S MESSAGES:
********************************************************************
Hi Everyone! We would like to invite everyone to migrate to the new
and improved git server, gitlab.svail.baidu.com! Please see the
wiki for information or message us for questions and how-tos:
http://wiki.svail.baidu.com/doku/doku.php?id=cluster:ssap:migrate_github_to_gitlab
********************************************************************
...
EOF
Last login: Mon Dec 16 18:04:09 2019 from 172.19.16.37
(base) sajjadwasim@asimov-3:~$ clear

(base) sajjadwasim@asimov-3:~$ cd /mnt/admin/scripts/
(base) sajjadwasim@asimov-3:/mnt/admin/scripts$ ls
add_svail_certs.bash         node_power_control.bash
Bad_Robot_check              node_up.bash
card_identify.bash           obsolete
check_nvidia_throttles.bash  quick_check_node_epilog_problem.bash
create_icinga_user.bash      quick_cleanup_apt_keys.bash
create_user_global.bash      quick_nvidia_node_up_fix.bash
create_user_nis_only.bash    quick_task.bash
diskquota                    slurm_node_restart.bash
drain_node                   tmp
extend_slurmjob.sh           write_parallel.bash
(base) sajjadwasim@asimov-3:/mnt/admin/scripts$ cat create_user_global.bash
#!/bin/bash -euE
...
function get_next_uid() {
    ...
    debug echo "getting next available UID for user"
...
function create_ipa_automounts()
{
    ...
    declare -A _opts_home_array_=( ['default']="-fstype=nfs4,vers=4.0,rw,sec=sys,soft" ['compute_node_v4']="-fstype=nfs4,vers=4.2,rw,sec=sys,soft" ['interactive_node_v4']="-fstype=nfs4,vers=4.2,rw,sec=sys,soft")
    declare -A _opts_scrt_array_=( ['default']="-fstype=nfs4,vers=4.0,rw,sec=sys,soft" ['compute_node_v4']="-fstype=nfs4,vers=4.2,rw,sec=sys,soft" ['interactive_node_v4']="-fstype=nfs4,vers=4.2,rw,sec=sys,soft")
    declare -A _opts_data_array_=( ['default']="-fstype=nfs4,vers=4.1,rw,sec=sys,soft,noatime" ['compute_node_v4']="-fstype=nfs4,vers=4.2,rw,sec=sys,soft,noatime" ['interactive_node_v4']="-fstype=nfs4,vers=4.2,rw,sec=sys,soft,noatime")
    declare -A _opts_arch_array_=( ['default']="-fstype=nfs4,vers=4.2,ro,noatime,soft" ['compute_node_v4']="-fstype=nfs4,vers=4.2,ro,noatime,soft" ['interactive_node_v4']="-fstype=nfs4,vers=4.2,rw,noatime,soft")
    ...
    for _location_ in ${_locations_[@]}; do
        local _mount_opts_home_=${_opts_home_array_["${_location_}"]}
        local _mount_opts_scrt_=${_opts_scrt_array_["${_location_}"]}
        local _mount_opts_data_=${_opts_data_array_["${_location_}"]}
        local _mount_opts_arch_=${_opts_arch_array_["${_location_}"]}

        _user_exists_="user_exists_${_auth_host_}"
        if ${_GLOBALS_[${_user_exists_}]}; then
            debug echo "autofs entries: setting to automountkey-mod as user exists"
            local _auto_cmd_="automountkey-mod"
        else
            debug echo "autofs entries: keeping automountkey-add as user does not exist yet"
        fi
        if ${_GLOBALS_['test_run']}; then
            echo "Testrun: skipping create ipa automounts"
            continue
        fi
        if ${_GLOBALS_['dry_run']}; then
            echo "I would have run:"
            echo "/usr/bin/ssh ${_ipahost_} \"/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \'${_mount_opts_home_} ${_GLOBALS_['home_server']}-ib:/home/${_username_}\' auto.home\" "
            echo "/usr/bin/ssh ${_ipahost_} \"/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \'${_mount_opts_scrt_} ${_GLOBALS_['scratch_server']}-ib:/scratch/${_username_}\' auto.scratch\" "
            echo "/usr/bin/ssh ${_ipahost_} \"/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \'${_mount_opts_data_} ${_GLOBALS_['data_server']}-ib:/storage-8/data/${_username_}\' auto.data\" "
            echo "/usr/bin/ssh ${_ipahost_} \"/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \'${_mount_opts_arch_} ${_GLOBALS_['archive_server']}-ib:/archive/${_username_}\' auto.archive\" "
            continue
        fi
        ## only get here by failing above if statements...
        debug echo "Creating autofs entries in ipa..."
        /usr/bin/ssh ${_ipahost_} "/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \"${_mount_opts_home_} ${_GLOBALS_['home_server']}-ib:/home/${_username_}\" auto.home"
        /usr/bin/ssh ${_ipahost_} "/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \"${_mount_opts_scrt_} ${_GLOBALS_['scratch_server']}-ib:/scratch/${_username_}\" auto.scratch"
        /usr/bin/ssh ${_ipahost_} "/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \"${_mount_opts_data_} ${_GLOBALS_['data_server']}-ib:/storage-8/data/${_username_}\" auto.data"
        /usr/bin/ssh ${_ipahost_} "/usr/bin/kinit admin -k -t ${_keytab_}; ipa ${_auto_cmd_} ${_location_} --key ${_username_} --info \"${_mount_opts_arch_} ${_GLOBALS_['archive_server']}-ib:/archive/${_username_}\" auto.archive"
    done
}
...
function create_ipa_user() {
    ...
    if ${_GLOBALS_["user_exists_${_auth_host_}"]}; then
        debug echo "skipping user ${_username_}: already in IPA"
    elif ${_GLOBALS_['test_run']}; then
        echo "Testrun: skipping create ipa user..."
    elif ${_GLOBALS_['dry_run']}; then
        echo "I would have run:"
        echo "/usr/bin/ssh ${_ipahost_} \" /usr/bin/kinit admin -k -t ${_keytab_}; echo ${_password_} | /usr/bin/ipa user-add ${_username_} --password --uid=${_uid_} --gidnumber=${_gidnumber_} --gecos=\\\"${_gecos_}\\\" --first=\\\"${_first_}\\\" --last=\\\"${_last_}\\\" --email=\\\"${_email_}\\\" --homedir=${_homedir_} --shell=${_shell_} --noprivate"
    else
        debug echo "creating user in ipa"
...
#    local _archive_server_="10.78.48.176"
    ...
    if ${_GLOBALS_['dry_run']}; then
        echo "I would have run:"
        echo "/usr/bin/ssh ${_nis_server_} \"echo ${_username_} ${_mount_opts_home_} ${_home_server_}:/home/${_username_} | tee --append ${_auto_home_}\" "
#        echo "/usr/bin/ssh ${_nis_server_} \"echo ${_username_} ${_mount_opts_scrt_} ${_scratch_server_}:/scratch/${_username_} | tee --append ${_auto_scratch_}\" "
#        echo "/usr/bin/ssh ${_nis_server_} \"echo ${_username_} ${_mount_opts_data_} ${_data_server_}:/storage-8/data/${_username_} | tee --append ${_auto_data_}\" "
#        echo "/usr/bin/ssh ${_nis_server_} \"echo ${_username_} ${_mount_opts_arch_} ${_archive_server_}:/archive/${_username_} | tee --append ${_auto_archive_}\" "
    elif ${_GLOBALS_['test_run']}; then
        echo "Testrun: skipping create nis automounts"
    else
        # do a sanity test first...
        debug echo "creating mounts on ${_nis_server_}"
...
function create_nis_user() {
    ...
    if ${_GLOBALS_["user_exists_${_auth_host_}"]}; then
        debug echo "skipping user ${_username_}: already in NIS"
    elif ${_GLOBALS_['test_run']}; then
        echo "Testrun: skipping create nis user"
    elif ${_GLOBALS_['dry_run']}; then
       echo "I would have run:"
       echo "/usr/bin/ssh ${_nis_server_} \"echo -e \'${_shadow_string_}\' | tee --append ${_shadow_file_}\" "
       echo "/usr/bin/ssh ${_nis_server_} \"echo -e \\\"${_passwd_string_}\\\" | tee --append ${_passwd_file_}\" "
       echo "/usr/bin/ssh ${_nis_server_} \"pushd /var/yp; make -C /var/yp; popd\" "
    else
        debug echo "creating new shadow entry in NIS"
        /usr/bin/ssh ${_nis_server_} "echo -e '${_shadow_string_}' | tee --append ${_shadow_file_}"

        debug echo "creating new passwd entry in NIS"
...
function create_nfs_mounts() {
    ...
    if ${_GLOBALS_['dry_run']}; then
        ...
        echo "Testrun: skipping create nfs mounts"
    else
        debug echo "creating directories on nfs servers and applying quotas"

        debug echo "creating home ${home_mount:-}/${_username_:-} on ${_home_server_:-}"
        /usr/bin/ssh ${_home_server_:-localhost} "${create_home_command}"
        debug echo "creating home ${scratch_mount:-}/${_username_:-} on ${_scratch_server_:-}"
        /usr/bin/ssh ${_scratch_server_:-localhost} "${create_scratch_command}"
        debug echo "creating home ${data_mount:-}/${_username_:-} on ${_data_server_:-}"
        /usr/bin/ssh ${_data_server_:-localhost} "${create_data_command}"
        debug echo "creating home ${archive_mount:-}/${_username_:-} on ${_archive_server_:-}"
...
function create_slurm_user() {
    ...
    if [[ ${cluster} == "asimov" ]]; then
        if ${_GLOBALS_['dry_run']}; then
            ...
            echo "Testrun: skipping create slurm user asimov"
        else
            debug echo "Adding user to asimov slurm"
        ...
        if ${_GLOBALS_['dry_run']}; then
            ...
            echo "Testrun: skipping create slurm user svail"
        else
...
success
#___END___#
(base) sajjadwasim@asimov-3:/mnt/admin/scripts$ vi create_user_global.bash
